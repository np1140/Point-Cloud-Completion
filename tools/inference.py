...
def inference_single(model, pc_path, args, config, root=None):
    ...
    if config.dataset.train._base_['NAME'] == 'ShapeNet' or 'Liver':
    ...
    if config.dataset.train._base_['NAME'] == 'ShapeNet' or 'Liver':
        ...
        input_points = pc_ndarray_normalized['input'] * m
        input_points = input_points + centroid

    if args.out_pc_root != '':
        target_path = os.path.join(args.out_pc_root, os.path.splitext(pc_path)[0])
        os.makedirs(target_path, exist_ok=True)
        point_cloud_o3d = o3d.geometry.PointCloud()
        point_cloud_o3d.points = o3d.utility.Vector3dVector(input_points.numpy())
        o3d.io.write_point_cloud(target_path + "/input.ply", point_cloud_o3d)
        point_cloud_o3d = o3d.geometry.PointCloud()
        point_cloud_o3d.points = o3d.utility.Vector3dVector(dense_points)
        o3d.io.write_point_cloud(target_path + "/fine.ply", point_cloud_o3d)
