...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data, noise) in enumerate(train_dataloader):
            ...
            elif dataset_name == 'ShapeNet' or 'Liver':
                trot = None
                if args.rot == 'z':
                        trot = RotateAxisAngle(angle=torch.rand(data.shape[0])*360, axis="Z", degrees=True)
                elif args.rot == 'so3':
                    trot = Rotate(R=random_rotations(data.shape[0]))
                if trot is not None:
...
def test(base_model, test_dataloader, ChamferDisL1, ChamferDisL2, args, config, logger = None):
    ...
    n_samples = len(test_dataloader) # bs is 1
    count = 1
    header = ['F-score', 'CDl1', 'CDl2', 'EMD']

    with torch.no_grad():
        with open('stats/Pointr_easy_so3.csv', 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(header)

            for idx, (taxonomy_ids, model_ids, data, noise) in enumerate(test_dataloader):
                taxonomy_id = taxonomy_ids[0] if isinstance(taxonomy_ids[0], str) else taxonomy_ids[0].item()
                model_id = model_ids[0]
                ...
                if dataset_name == 'PCN' or dataset_name == 'Projected_ShapeNet':
                    ...
                    # point_cloud_o3d.points = o3d.utility.Vector3dVector(gt.cpu().squeeze(0).numpy())
                    # o3d.io.write_point_cloud('inference_result/adapointr/01149/medianDef' + f"/gt{count}.ply", point_cloud_o3d)                
                    # print('Noise', noise)
                    # noise = noise.cuda()
                    for item in choice:           
                        ...
                        # noise = np.random.normal(loc=0, scale=0.5, size=partial.shape)  # 0.1 is an example standard deviation
                        ...
                        _metrics = Metrics.get(dense_points ,gt)
                        # point_cloud_o3d = o3d.geometry.PointCloud()
                        # point_cloud_o3d.points = o3d.utility.Vector3dVector(partial.cpu().squeeze(0).numpy())
                        # o3d.io.write_point_cloud('inference_result/adapointr/01149/medianDef' + f"/input{count}.ply", point_cloud_o3d)
                        # point_cloud_o3d = o3d.geometry.PointCloud()
                        # point_cloud_o3d.points = o3d.utility.Vector3dVector(dense_points.cpu().squeeze(0).numpy())
                        # o3d.io.write_point_cloud('inference_result/adapointr/01149/medianDef' + f"/fine{count}.ply", point_cloud_o3d)
                        # count = count + 1
                        ...
                        if taxonomy_id not in category_metrics:
                            category_metrics[taxonomy_id] = AverageMeter(Metrics.names())
                        category_metrics[taxonomy_id].update(_metrics)
                        metrics_row = [x.item() for x in _metrics]
