...
class PCTransformer(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=768, depth=[6, 6], num_heads=6, mlp_ratio=2., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        self.grouper = EQCNN_test() #DGCNN_Grouper()  # B 3 N to B C(3) N(128) and B C(128) N(128)
    ...
    def forward(self, inpc):
        ...
        # build point proxy
        bs = inpc.size(0)
        # coor2,f2 = self.grouper2(inpc.transpose(1,2).contiguous())
        # print("f2 shape",f2.shape)
        # print("Coor2 shape",coor2.shape)
        # print("Input shape",inpc.shape)
        coor,f = self.grouper(inpc.transpose(1,2).contiguous()) #coor,f
        # print("f shape",f.shape)
        knn_index = get_knn_index(coor2)
